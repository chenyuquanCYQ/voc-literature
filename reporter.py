...
def generate_daily_report(conn, report_dir: str,
    ...
    lines = [
        f"# VOC 文獻監控日報 — {today}",
        "",
        f"> 本次執行：{datetime.now().strftime('%H:%M:%S')} | "
        f"新增 **{new_count}** 篇 | 其中相關 **{len(relevant)}** 篇",
        "",
        "---",
        "",
        "## 📊 本次新增概覽",
        ...
        f"| 資料庫累計總數 | {stats_row[0] or 0} |",
        f"| 資料庫相關總數 | {stats_row[1] or 0} |",
        f"| 相關文獻平均分 | {stats_row[2] or 0} |",
        "",
    ]
    ...
    if irrelevant:
        lines += [
            "---",
            f"## ℹ️ 本次排除文獻：{len(irrelevant)} 篇（不相關或低相關）",
            "",
        ]
